        ...
        <div className="bg-cream-50 min-h-screen py-16">
          <div className="container mx-auto px-4">
            <div className="empty-cart-container text-center">
              ...
              />
              <h2 className="empty-cart-title text-2xl font-semibold mt-4 text-gray-200">Your cart is empty</h2>
              <p className="empty-cart-text text-sm mb-6 text-gray-300">
                Looks like you haven't added any items to your cart yet.
              </p>
              <Link
                href="/menu"
                className="browse-menu-button text-white bg-primary rounded-lg px-6 py-3 hover:bg-primary-dark"
              >
                Browse Menu
              </Link>
            </div>
              ...
              <motion.div className="space-y-6" variants={container} initial="hidden" animate="show">
                {cart.map((item) => (
                  <motion.div
                    key={item.id}
                    variants={item}
                    className="card p-4 bg-white shadow-md rounded-lg hover:shadow-xl transition-shadow duration-300"
                  >
                    <div className="card-content flex flex-col lg:flex-row items-center lg:items-start space-y-4 lg:space-y-0 lg:space-x-6">
                      ...
                      <div className="card-details flex flex-col space-y-2">
                        <h3 className="card-title text-lg font-medium text-gray-200">{item.name}</h3>
                        <p className="card-price text-base font-semibold text-gray-300">${item.price.toFixed(2)}</p>
